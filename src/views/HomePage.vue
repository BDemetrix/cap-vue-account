<template>
  <div class="login wrapper">
    ...
    <button class="btn" type="button" @click="sendPush">{{btnText}}</button>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      PUBLIC_VAPID_KEY: "BP18qEecjZuO4Yi_xtG6jlmqk7EHGCqpc46zH6cuanWlqL7nXpKyfPJmmToFqdxD55rENrF_STkXjoX4U4KOxJw"
    };
  },
  props: {
    firebaseConfig: {
  ...
  methods: {
    async sendPush() {
      ...
      const token = await getToken(messaging, { vapidKey: this.PUBLIC_VAPID_KEY });
      console.log(token);
      window.localStorage.setItem('fcmSigned', '1')
      this.fcmSigned = true
      this.btnText = this.fcmSigned ? 'Отправить Push' : 'Подписаться на Push'
